...
describe ClearancingService do
  ...
  describe "::process_item" do
    ...
    context "total success new batch" do
      ...
      before(:each) { @clearance_service = ClearancingService.new(item_id: item.id, batch: nil) }
    ...
    context "total success existing batch" do
      ...
      let(:item) { FactoryBot.create(:item) }
      before(:each) { @clearance_service = ClearancingService.new(item_id: item.id, batch: batch) }
    ...
    context "total failure with batch" do
      ...
      it 'handles invalid input' do
        @clearance_service = ClearancingService.new(item_id: 'A', batch: batch)
        expect(@clearance_service.errors).to include "Item id 0 is not valid"
      end

      it 'handles item not found' do
        @clearance_service = ClearancingService.new(item_id: 1234, batch: batch)
      ...
      it 'handles an item already clearanced' do
        ...
        @clearance_service = ClearancingService.new(item_id: existing_item.id, batch: batch)
      ...
      it 'handles an item not sellable' do
        ...
        @clearance_service = ClearancingService.new(item_id: sold_item.id, batch: batch)
    ...
    context "total failure without batch" do
      ...
      it 'handles invalid input' do
        @clearance_service = ClearancingService.new(item_id: 'A', batch: nil)
        expect(@clearance_service.errors).to include "Item id 0 is not valid"
      end

      it 'handles item not found' do
        @clearance_service = ClearancingService.new(item_id: 1234, batch: nil)
        expect(@clearance_service.errors).to include "Item id 1234 could not be found"
      end

      it 'handles an item already clearanced' do
        existing_item = FactoryBot.create(:clearance_item)
        @clearance_service = ClearancingService.new(item_id: existing_item.id, batch: nil)
        expect(@clearance_service.errors).to include "Item id #{existing_item.id} already clearanced"
      end

      it 'handles an item not sellable' do
        sold_item = FactoryBot.create(:item, status: 'sold')
        @clearance_service = ClearancingService.new(item_id: sold_item.id, batch: nil)
        expect(@clearance_service.errors).to include "Item id #{sold_item.id} could not be clearanced"
      end
  ...
  describe "::process_file" do
    ...
    context "total success" do
      ...
      before do
        @clearance_service = ClearancingService.new(file: uploaded_file)
      end
